...
(function () {
	...
	window.lifecycle = {
		afterEach: function () {
			// Remove the cookies created using js-cookie-calibre default attributes
			Object.keys(Calibre.get()).forEach(Calibre.remove);
			// Remove the cookies created using browser default attributes
			Object.keys(Calibre.get()).forEach(function (cookie) {
				Calibre.remove(cookie, {
					path: ''
				});
			});
		}
		...
		function setCookie(name, value) {
			return {
				then: function (callback) {
					...
					if (!serverURL) {
						callback(Calibre.get(name), document.cookie);
					} else {
						var requestURL = [
							serverURL,
							'encoding?',
